...
class _DiagnosisTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      padding: const EdgeInsets.all(20),
      children: [
        _DiagnosisCard(diagnosis: liveDiagnosis!),
        const SizedBox(height: 20),
        
        // Tombol interaktif untuk menyembuhkan tanaman langsung jika statusnya sedang sakit/karantina
        if (plant.status == PlantStatus.quarantine)
          ElevatedButton.icon(
            onPressed: onMarkAsHealthy,
            icon: const Icon(Icons.check_circle_outline_rounded, size: 18),
            label: const Text('Nyatakan Sembuh (Set Sehat)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
